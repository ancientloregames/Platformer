...
public class GOPlayer extends GameObject {
    ...
    GOPlayer(Context context, float worldStartX, float worldStartY, int pixelsPerMetre) {

        final float HEIGHT = 2;
        final float WIDTH = 1;

        bfg = new GOUtilityGun();
        ...
        // In chapter 6.
        setBitmapName("platformer_player");
    ...
    public void update(long fps, float gravity) {
        ...
        rectHitboxLeft.top = ly + getHeight() * .4f;
        rectHitboxLeft.left = lx + getWidth() * .2f;
        rectHitboxLeft.bottom = ly + getHeight() * .6f;
        rectHitboxLeft.right = lx + (getWidth() * .3f);

        // Update player right hitbox
        rectHitboxRight.top = ly + getHeight() * .4f;
        rectHitboxRight.left = lx + (getWidth() * .8f);
        rectHitboxRight.bottom = ly + getHeight() * .6f;
    ...
    public void startJump(ManagerSound sm) {
