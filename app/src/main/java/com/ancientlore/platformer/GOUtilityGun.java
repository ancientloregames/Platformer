...
public class GOUtilityGun extends GameObject{
    ...
    private CopyOnWriteArrayList<GOUtilityBullet> bullets;

    int speed = 25;

    GOUtilityGun(){
        bullets = new CopyOnWriteArrayList<GOUtilityBullet>();
        lastShotTime = -1;
        nextBullet = -1;
    }
    ...
    public void update(long fps, float gravity){
        ...
        for(GOUtilityBullet bullet: bullets){
    ...
    public boolean shoot(float ownerX, float ownerY, int ownerFacing, float ownerHeight){
        ...
        if(System.currentTimeMillis() - lastShotTime  > 1000/rateOfFire){
            ...
            bullets.add(nextBullet, new GOUtilityBullet(ownerX, (ownerY+ ownerHeight/3), speed, ownerFacing));
            shotFired = true;
            numBullets++;
        }
        return shotFired;
